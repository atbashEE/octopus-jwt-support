...
public class RSAKeyTest {
    ...
    public void testConstructAndSerialize()
        ...
        assertThat(key.getModulus()).isEqualTo(new Base64URLValue(n));
        assertThat(key.getPublicExponent()).isEqualTo(new Base64URLValue(e));

        assertThat(key.getPrivateExponent()).isEqualTo(new Base64URLValue(d));

        assertThat(key.getFirstPrimeFactor()).isEqualTo(new Base64URLValue(p));
        assertThat(key.getSecondPrimeFactor()).isEqualTo(new Base64URLValue(q));

        assertThat(key.getFirstFactorCRTExponent()).isEqualTo(new Base64URLValue(dp));
        assertThat(key.getSecondFactorCRTExponent()).isEqualTo(new Base64URLValue(dq));

        assertThat(key.getFirstCRTCoefficient()).isEqualTo(new Base64URLValue(qi));

        assertThat(key.getOtherPrimes().isEmpty()).isTrue();

        // private key generated from key material, not PrivateKey ref
        assertThat(key.toPrivateKey()).isNotSameAs(privateKey);
        ...
        assertThat(key.getPrivateExponent()).isNull();

        assertThat(key.getFirstPrimeFactor()).isNull();
        assertThat(key.getSecondPrimeFactor()).isNull();

        assertThat(key.getFirstFactorCRTExponent()).isNull();
        assertThat(key.getSecondFactorCRTExponent()).isNull();

        assertThat(key.getFirstCRTCoefficient()).isNull();
    ...
    public void testObjectBuilder()
        ...
        assertThat(publicKey.getModulus().equals(key.getModulus().decodeToBigInteger())).isTrue();
        assertThat(publicKey.getPublicExponent().equals(key.getPublicExponent().decodeToBigInteger())).isTrue();

        assertThat(privateKey.getPrivateExponent().equals(key.getPrivateExponent().decodeToBigInteger())).isTrue();
    ...
    public void testRSAPublicKeyExportAndImport()
        ...
        assertThat(pubKey.getModulus()).isEqualTo(new Base64URLValue(n).decodeToBigInteger());
        assertThat(pubKey.getPublicExponent()).isEqualTo(new Base64URLValue(e).decodeToBigInteger());
    ...
    public void testRSAPrivateKeyExportAndImport()
        ...
        assertThat(privCrtKey.getPrimeQ()).isEqualTo(new Base64URLValue(q).decodeToBigInteger());
        assertThat(privCrtKey.getPrimeExponentP()).isEqualTo(new Base64URLValue(dp).decodeToBigInteger());
        assertThat(privCrtKey.getPrimeExponentQ()).isEqualTo(new Base64URLValue(dq).decodeToBigInteger());
        assertThat(privCrtKey.getCrtCoefficient()).isEqualTo(new Base64URLValue(qi).decodeToBigInteger());
    ...
    public void testParseSomeKey()
        ...
        assertThat(key.getAlgorithm()).isEqualTo(JWSAlgorithm.RS256);
        assertThat(key.getModulus().decode().length).isEqualTo(256);
    }
    ...
    public void testKeyConversionRoundTrip()
        ...
        assertThat(rsaJWK.getPublicExponent().decodeToBigInteger()).isEqualTo(rsaPublicKeyIn.getPublicExponent());
        assertThat(rsaJWK.getModulus().decodeToBigInteger()).isEqualTo(rsaPublicKeyIn.getModulus());
        assertThat(rsaJWK.getPrivateExponent().decodeToBigInteger()).isEqualTo(rsaPrivateKeyIn.getPrivateExponent());
        ...
        assertThat(rsaPublicKeyOut.getModulus()).isEqualTo(rsaPublicKeyIn.getModulus());

        assertThat(rsaPrivateKeyOut.getAlgorithm()).isEqualTo(rsaPrivateKeyIn.getAlgorithm());
        assertThat(rsaPrivateKeyOut.getPrivateExponent()).isEqualTo(rsaPrivateKeyIn.getPrivateExponent());

        // Compare encoded forms
        assertThat(Base64Value.encode(rsaPublicKeyOut.getEncoded()).toString()).withFailMessage("Public RSA").isEqualTo(Base64Value.encode(rsaPublicKeyIn.getEncoded()).toString());
        assertThat(Base64Value.encode(rsaPrivateKeyOut.getEncoded()).toString()).withFailMessage("Private RSA").isEqualTo(Base64Value.encode(rsaPrivateKeyIn.getEncoded()).toString());

        RSAKey rsaJWK2 = new RSAKey.Builder(rsaPublicKeyOut).privateKey(rsaPrivateKeyOut).build();

        // Compare JWK values with original Java RSA values
        assertThat(rsaJWK2.getPublicExponent().decodeToBigInteger()).isEqualTo(rsaPublicKeyIn.getPublicExponent());
        assertThat(rsaJWK2.getModulus().decodeToBigInteger()).isEqualTo(rsaPublicKeyIn.getModulus());
        assertThat(rsaJWK2.getPrivateExponent().decodeToBigInteger()).isEqualTo(rsaPrivateKeyIn.getPrivateExponent());
    }
    ...
    public void testKeyConversionRoundTripWithCRTParams()
        ...
        // Compare CRT params
        assertThat(rsaJWK.getFirstPrimeFactor().decodeToBigInteger()).isEqualTo(rsaPrivateKeyIn.getPrimeP());
        assertThat(rsaJWK.getSecondPrimeFactor().decodeToBigInteger()).isEqualTo(rsaPrivateKeyIn.getPrimeQ());
        assertThat(rsaJWK.getFirstFactorCRTExponent().decodeToBigInteger()).isEqualTo(rsaPrivateKeyIn.getPrimeExponentP());
        assertThat(rsaJWK.getSecondFactorCRTExponent().decodeToBigInteger()).isEqualTo(rsaPrivateKeyIn.getPrimeExponentQ());
        assertThat(rsaJWK.getFirstCRTCoefficient().decodeToBigInteger()).isEqualTo(rsaPrivateKeyIn.getCrtCoefficient());
        assertThat(rsaJWK.getOtherPrimes() == null || rsaJWK.getOtherPrimes().isEmpty()).isTrue();
        ...
        assertThat(rsaPrivateKeyOut.getPrimeP()).isEqualTo(rsaPrivateKeyIn.getPrimeP());
        assertThat(rsaPrivateKeyOut.getPrimeQ()).isEqualTo(rsaPrivateKeyIn.getPrimeQ());
        assertThat(rsaPrivateKeyOut.getPrimeExponentP()).isEqualTo(rsaPrivateKeyIn.getPrimeExponentP());
        assertThat(rsaPrivateKeyOut.getPrimeExponentQ()).isEqualTo(rsaPrivateKeyIn.getPrimeExponentQ());
        assertThat(rsaPrivateKeyOut.getCrtCoefficient()).isEqualTo(rsaPrivateKeyIn.getCrtCoefficient());
        ...
        // Compare CRT params
        assertThat(rsaJWK2.getFirstPrimeFactor().decodeToBigInteger()).isEqualTo(rsaPrivateKeyIn.getPrimeP());
        assertThat(rsaJWK2.getSecondPrimeFactor().decodeToBigInteger()).isEqualTo(rsaPrivateKeyIn.getPrimeQ());
        assertThat(rsaJWK2.getFirstFactorCRTExponent().decodeToBigInteger()).isEqualTo(rsaPrivateKeyIn.getPrimeExponentP());
        assertThat(rsaJWK2.getSecondFactorCRTExponent().decodeToBigInteger()).isEqualTo(rsaPrivateKeyIn.getPrimeExponentQ());
        assertThat(rsaJWK2.getFirstCRTCoefficient().decodeToBigInteger()).isEqualTo(rsaPrivateKeyIn.getCrtCoefficient());
        assertThat(rsaJWK2.getOtherPrimes() == null || rsaJWK2.getOtherPrimes().isEmpty()).isTrue();
    }
    ...
    public void testKeyUseConsistentWithOps() {
        ...
        JWK jwk = new RSAKey(new Base64URLValue(n), new Base64URLValue(e), use, ops, null, null, null, null, null, null, null);
    ...
    public void testParseCookbookExample()
        ...
        assertThat(jwk.getModulus().toString()).isEqualTo("n4EPtAOCc9AlkeQHPzHStgAbgs7bTZLwUBZdR8_KuKPEHLd4rHVTeT" +
                "-O-XV2jRojdNhxJWTDvNd7nqQ0VEiZQHz_AJmSCpMaJMRBSFKrKb2wqV" +
                "wGU_NsYOYL-QtiWN2lbzcEe6XC0dApr5ydQLrHqkHHig3RBordaZ6Aj-" +
                "oBHqFEHYpPe7Tpe-OfVfHd1E6cS6M1FZcD1NNLYD5lFHpPI9bTwJlsde" +
                "3uhGqC0ZCuEHg8lhzwOHrtIQbS0FVbb9k3-tVTU4fg_3L_vniUFAKwuC" +
        ...
        assertThat(jwk.getPublicExponent().toString()).isEqualTo("AQAB");

        assertThat(jwk.getPrivateExponent().toString()).isEqualTo("bWUC9B-EFRIo8kpGfh0ZuyGPvMNKvYWNtB_ikiH9k20eT-O1q_I78e" +
                "iZkpXxXQ0UTEs2LsNRS-8uJbvQ-A1irkwMSMkK1J3XTGgdrhCku9gRld" +
                "Y7sNA_AKZGh-Q661_42rINLRCe8W-nZ34ui_qOfkLnK9QWDDqpaIsA-b" +
                "MwWWSDFu2MUBYwkHTMEzLYGqOe04noqeq1hExBTHBOBdkMXiuFhUq1BU" +
                "6l-DqEiWxqg82sXt2h-LMnT3046AOYJoRioz75tSUQfGCshWTBnP5uDj" +
        ...
        assertThat(jwk.getFirstPrimeFactor().toString()).isEqualTo("3Slxg_DwTXJcb6095RoXygQCAZ5RnAvZlno1yhHtnUex_fp7AZ_9nR" +
                "aO7HX_-SFfGQeutao2TDjDAWU4Vupk8rw9JR0AzZ0N2fvuIAmr_WCsmG" +
                "peNqQnev1T7IyEsnh8UMt-n5CafhkikzhEsrmndH6LxOrvRJlsPp6Zv8" +
                "bUq0k");

        assertThat(jwk.getSecondPrimeFactor().toString()).isEqualTo("uKE2dh-cTf6ERF4k4e_jy78GfPYUIaUyoSSJuBzp3Cubk3OCqs6grT" +
                "8bR_cu0Dm1MZwWmtdqDyI95HrUeq3MP15vMMON8lHTeZu2lmKvwqW7an" +
                "V5UzhM1iZ7z4yMkuUwFWoBvyY898EXvRD-hdqRxHlSqAZ192zB3pVFJ0" +
                "s7pFc");

        assertThat(jwk.getFirstFactorCRTExponent().toString()).isEqualTo("B8PVvXkvJrj2L-GYQ7v3y9r6Kw5g9SahXBwsWUzp19TVlgI-YV85q" +
                "1NIb1rxQtD-IsXXR3-TanevuRPRt5OBOdiMGQp8pbt26gljYfKU_E9xn" +
                "-RULHz0-ed9E9gXLKD4VGngpz-PfQ_q29pk5xWHoJp009Qf1HvChixRX" +
                "59ehik");

        assertThat(jwk.getSecondFactorCRTExponent().toString()).isEqualTo("CLDmDGduhylc9o7r84rEUVn7pzQ6PF83Y-iBZx5NT-TpnOZKF1pEr" +
                "AMVeKzFEl41DlHHqqBLSM0W1sOFbwTxYWZDm6sI6og5iTbwQGIC3gnJK" +
                "bi_7k_vJgGHwHxgPaX2PnvP-zyEkDERuf-ry4c_Z11Cq9AqC2yeL6kdK" +
                "T1cYF8");

        assertThat(jwk.getFirstCRTCoefficient().toString()).isEqualTo("3PiqvXQN0zwMeE-sBvZgi289XP9XCQF3VWqPzMKnIgQp7_Tugo6-N" +
                "ZBKCQsMf3HaEGBjTVJs_jcK8-TRXvaKe-7ZMaQj8VfBdYkssbu0NKDDh" +
                "jJ-GtiseaDVWt7dcH0cfwxgFUHpQh7FoCrjFJ6h6ZEpMF6xmujs4qMpP" +
                "z8aaI4");
    ...
    public void testParseCookbookExample2()
        ...
        assertThat(jwk.getKeyType()).isEqualTo(KeyType.RSA);
        assertThat(jwk.getKeyID()).isEqualTo("frodo.baggins@hobbiton.example");
        assertThat(jwk.getKeyUse()).isEqualTo(KeyUse.ENCRYPTION);

        assertThat(jwk.getModulus().toString()).isEqualTo("maxhbsmBtdQ3CNrKvprUE6n9lYcregDMLYNeTAWcLj8NnPU9XIYegT" +
                "HVHQjxKDSHP2l-F5jS7sppG1wgdAqZyhnWvXhYNvcM7RfgKxqNx_xAHx" +
                "6f3yy7s-M9PSNCwPC2lh6UAkR4I00EhV9lrypM9Pi4lBUop9t5fS9W5U" +
                "NwaAllhrd-osQGPjIeI1deHTwx-ZTHu3C60Pu_LJIl6hKn9wbwaUmA4c" +
                "R5Bd2pgbaY7ASgsjCUbtYJaNIHSoHXprUdJZKUMAzV0WOKPfA6OPI4oy" +
        ...
        assertThat(jwk.getPrivateExponent().toString()).isEqualTo("Kn9tgoHfiTVi8uPu5b9TnwyHwG5dK6RE0uFdlpCGnJN7ZEi963R7wy" +
                "bQ1PLAHmpIbNTztfrheoAniRV1NCIqXaW_qS461xiDTp4ntEPnqcKsyO" +
                "5jMAji7-CL8vhpYYowNFvIesgMoVaPRYMYT9TW63hNM0aWs7USZ_hLg6" +
                "Oe1mY0vHTI3FucjSM86Nff4oIENt43r2fspgEPGRrdE6fpLc9Oaq-qeP" +
                "1GFULimrRdndm-P8q8kvN3KHlNAtEgrQAgTTgz80S-3VD0FgWfgnb1PN" +
        ...
        assertThat(jwk.getFirstPrimeFactor().toString()).isEqualTo("2DwQmZ43FoTnQ8IkUj3BmKRf5Eh2mizZA5xEJ2MinUE3sdTYKSLtaE" +
                "oekX9vbBZuWxHdVhM6UnKCJ_2iNk8Z0ayLYHL0_G21aXf9-unynEpUsH" +
                "7HHTklLpYAzOOx1ZgVljoxAdWNn3hiEFrjZLZGS7lOH-a3QQlDDQoJOJ" +
                "2VFmU");

        assertThat(jwk.getSecondPrimeFactor().toString()).isEqualTo("te8LY4-W7IyaqH1ExujjMqkTAlTeRbv0VLQnfLY2xINnrWdwiQ93_V" +
                "F099aP1ESeLja2nw-6iKIe-qT7mtCPozKfVtUYfz5HrJ_XY2kfexJINb" +
                "9lhZHMv5p1skZpeIS-GPHCC6gRlKo1q-idn_qxyusfWv7WAxlSVfQfk8" +
                "d6Et0");

        assertThat(jwk.getFirstFactorCRTExponent().toString()).isEqualTo("UfYKcL_or492vVc0PzwLSplbg4L3-Z5wL48mwiswbpzOyIgd2xHTH" +
                "QmjJpFAIZ8q-zf9RmgJXkDrFs9rkdxPtAsL1WYdeCT5c125Fkdg317JV" +
                "RDo1inX7x2Kdh8ERCreW8_4zXItuTl_KiXZNU5lvMQjWbIw2eTx1lpsf" +
                "lo0rYU");

        assertThat(jwk.getSecondFactorCRTExponent().toString()).isEqualTo("iEgcO-QfpepdH8FWd7mUFyrXdnOkXJBCogChY6YKuIHGc_p8Le9Mb" +
                "pFKESzEaLlN1Ehf3B6oGBl5Iz_ayUlZj2IoQZ82znoUrpa9fVYNot87A" +
                "CfzIG7q9Mv7RiPAderZi03tkVXAdaBau_9vs5rS-7HMtxkVrxSUvJY14" +
                "TkXlHE");

        assertThat(jwk.getFirstCRTCoefficient().toString()).isEqualTo("kC-lzZOqoFaZCr5l0tOVtREKoVqaAYhQiqIRGL-MzS4sCmRkxm5vZ" +
                "lXYx6RtE1n_AagjqajlkjieGlxTTThHD8Iga6foGBMaAr5uR1hGQpSc7" +
                "Gl7CF1DZkBJMTQN6EshYzZfxW08mIO8M6Rzuh0beL6fG9mkDcIyPrBXx" +
                "2bQ_mM");
    ...
    public void testComputeThumbprint()
        ...
        assertThat(rsaKey.computeThumbprint("SHA-256")).isEqualTo(thumbprint);

        assertThat(thumbprint.toString()).isEqualTo("NzbLsXh8uDCcd-6MNwXF4W_7noWXFZAfHkxZsRGC9Xs");
    ...
    public void testThumbprintAsKeyID()
        ...
        assertThat(rsaKey.getKeyID()).isEqualTo("NzbLsXh8uDCcd-6MNwXF4W_7noWXFZAfHkxZsRGC9Xs");
    }
    ...
    public void testThumbprintSHA1AsKeyID()
        ...
        assertThat(new Base64URLValue(rsaKey.getKeyID()).decode().length).isEqualTo(160 / 8);
    }

    @Test
    public void testSize() {

        assertThat(new RSAKey.Builder(new Base64URLValue(n), new Base64URLValue(e)).build().size()).isEqualTo(2048);
    }
    ...
    public void testPrivateKeyHandle()
        ...
        assertThat(rsaJWK.toPublicKey()).isNotNull();
        assertThat(rsaJWK.toPrivateKey()).isEqualTo(privateKey);
        assertThat(rsaJWK.isPrivate()).isTrue();
        ...
        assertThat(json.getString("kty")).isEqualTo("RSA");
        assertThat(json.getString("kid")).isEqualTo("1");
        assertThat(json.getString("e")).isEqualTo(Base64URLValue.encode(publicKey.getPublicExponent()).toString());
        assertThat(json.getString("n")).isEqualTo(Base64URLValue.encode(publicKey.getModulus()).toString());
    ...
    public void testParseFromX509Cert()
        ...
        assertThat(rsaKey.getKeyID()).isEqualTo(cert.getSerialNumber().toString(10));
        assertThat(rsaKey.getX509CertChain().get(0).toString()).isNotSameAs(pemEncodedCert);
        assertThat(rsaKey.getX509CertChain()).hasSize(1);
        assertThat(rsaKey.getX509CertThumbprint()).isNull();
        assertThat(rsaKey.getX509CertSHA256Thumbprint()).isEqualTo(Base64URLValue.encode(sha256.digest(cert.getEncoded())));
        assertThat(rsaKey.getAlgorithm()).isNull();
        assertThat(rsaKey.getKeyOperations()).isNull();
    }
    ...
    public void testX509CertificateChain()
        ...
        assertThat(jwk.getX509CertChain().get(0)).isEqualTo(SampleCertificates.SAMPLE_X5C_RSA.get(0));
        assertThat(jwk.getX509CertChain().get(1)).isEqualTo(SampleCertificates.SAMPLE_X5C_RSA.get(1));
        assertThat(jwk.getX509CertChain().get(2)).isEqualTo(SampleCertificates.SAMPLE_X5C_RSA.get(2));
    ...
    public void testLoadFromKeyStore_publicKeyOnly()
        ...
        assertThat(rsaKey.getKeyID()).isEqualTo("1");
        assertThat(rsaKey.getX509CertChain().size()).isEqualTo(1);
        assertThat(rsaKey.getParsedX509CertChain().size()).isEqualTo(1);
        assertThat(rsaKey.getX509CertThumbprint()).isNull();
        assertThat(rsaKey.getX509CertSHA256Thumbprint()).isNotNull();
    ...
    public void testEqualsFailure()
        ...
        //Then
        assertThat(keyA).isNotSameAs(keyB);
    ...
    public void testRSAKeyRoundtripWithX5c() throws Exception {
        ...
        assertThat(x5cArray.getString(0)).isEqualTo(rsaKey.getX509CertChain().get(0).toString());
        assertThat(x5cArray).hasSize(1);
