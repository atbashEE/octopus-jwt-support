...
. Support for setting 'crit' header values that are handled by applicaiton (understood by `JWSVerifier` implementations).
Possibility to pass on these header clams to `DefaultJWSVerifierFactory` ad `JWTDecoder`.
...
The `JWTEncoder` class can also be used for creating a JWE.
The difference is the parameter we supply, when encoding.
